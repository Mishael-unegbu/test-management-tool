...
describe('userStoriesService', () => {
  ...
  describe('createUserStory (US-005)', () => {
    it('assigns StoryID 1 to the first story and defaults Status to Open', async () => {
      const story = await userStoriesService.createUserStory(
        { projectId, title: 'Alpha Story', createdBy: 'Alice' },
        filePath
      );
    ...
    it('increments StoryID for subsequent stories', async () => {
      ...
      const second = await userStoriesService.createUserStory(
        { projectId, title: 'Beta', createdBy: 'Alice' },
        filePath
      );
  ...
  describe('searchUserStories (US-007)', () => {
    beforeEach(async () => {
      await userStoriesService.createUserStory(
        { projectId, title: 'Login page', status: 'Open', priority: 'P1', createdBy: 'Alice' },
        filePath
      );
      await userStoriesService.createUserStory(
        { projectId, title: 'Logout flow', status: 'In Progress', priority: 'P2', createdBy: 'Alice' },
        filePath
      );
      await userStoriesService.createUserStory(
        { projectId, title: 'Unrelated item', status: 'Closed', priority: 'P3', createdBy: 'Alice' },
        filePath
      );
    });
    ...
    it('filters by projectId', async () => {
      ...
      await userStoriesService.createUserStory(
        { projectId: otherProject.ProjectID, title: 'Other project story', createdBy: 'Alice' },
        filePath
      );
