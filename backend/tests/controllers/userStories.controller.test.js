...
describe('UserStories controller (via routes)', () => {
  ...
  describe('GET /api/projects/:projectId/user-stories', () => {
    it('returns 200 with all stories for the project', async () => {
      userStoriesService.getUserStoriesByProjectId.mockResolvedValue([
        { StoryID: 1, ProjectID: 1, Title: 'Login page' },
        { StoryID: 2, ProjectID: 1, Title: 'Logout flow' },
      ]);

      const res = await request(app).get('/api/projects/1/user-stories');
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(2);
      expect(res.body[0].ProjectID).toBe(1);
    });

    it('returns 200 with an empty array when the project has no stories', async () => {
      userStoriesService.getUserStoriesByProjectId.mockResolvedValue([]);
    ...
    it('returns 404 when the project does not exist', async () => {
      const err = new Error('Project with ProjectID "999" was not found.');
      err.statusCode = 404;
      userStoriesService.getUserStoriesByProjectId.mockRejectedValue(err);

      const res = await request(app).get('/api/projects/999/user-stories');
      expect(res.status).toBe(404);
